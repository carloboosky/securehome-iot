...
RETURNS TABLE(stream_url text, updated_at timestamptz, is_active boolean)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $function$
  SELECT camera.stream_url, camera.updated_at, camera.is_active
...
BEGIN
  ...
  SET is_active = target_is_active,
      ...
      updated_at = now()
  WHERE request_id = target_request_id
    AND stream_url = target_stream_url;
...
BEGIN
  IF NOT public.is_admin() THEN
    RAISE EXCEPTION 'No autorizado';-- REPARACIÓN DE ACCESO A CÁMARAS
-- Copia este archivo COMPLETO en Supabase > SQL Editor y pulsa Run.
-- No agregues instrucciones de RLS dentro de los bloques AS $$ ... $$.
