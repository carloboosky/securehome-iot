...
function failure(message, correlationId) {
  ...
  return { content: [{ type: "text", text: typeof payload === "string" ? payload : JSON.stringify(payload, null, 2) }], isError: true };
}

export function createSecureHomeServer() {
  ...
  if (!supabaseUrl || !serviceRoleKey) throw new Error("Faltan MCP_SUPABASE_URL o MCP_SUPABASE_SERVICE_ROLE_KEY.");

  const supabase = createClient(supabaseUrl, serviceRoleKey, { auth: { persistSession: false, autoRefreshToken: false } });
  const server = new McpServer({ name: "securehome-iot", version: "1.1.0" });
  ...
  server.registerTool("list-clients", {
    ...
    let query = supabase.from("service_requests").select("id,status,property_type,installation_address,created_at,profiles(full_name,phone),service_plans(name)").order("created_at", { ascending: false }).limit(limit);
  ...
  server.registerTool("get-security-overview", {
    ...
    try {
      ...
      if (firstError) { trace.finish("error", firstError.code || "DATABASE_ERROR"); return failure(`No se pudo crear el resumen: ${firstError.message}`, trace.correlationId); }
      const residentList = residents.data || [];
      const inferredHomeMode = residentList.length === 0 ? "SIN_RESIDENTES_REGISTRADOS" : residentList.every(person => !person.is_at_home) ? "AUSENTE" : "EN_CASA";
      trace.finish("success");
      return result({ request: request.data, activeCameras: cameras.data || [], residents: residentList, pets: pets.data || [], cameraDesign: design.data, inferredHomeMode }, trace.correlationId);
    } catch (error) {
